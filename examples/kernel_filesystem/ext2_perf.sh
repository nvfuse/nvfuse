...
for numjobs in {1..7}
do
    str="$FIO_PERF_PATH --name=test --directory=${MOUNT_PATH}/ --size=16g --ioengine=sync --bs=4k --rw=randwrite --numjobs=$numjobs --thread=1 --fsync=1  --group_reporting --minimal --runtime=120 --output=${OUTPUT_PATH}/kernel_ext2_sync_numjobs_${numjobs}_q_${qdepth}_block_${block_size}_workload_${workload}.log"
    echo $str
    eval $str
done
